...
//		Child child = (Child) parent;	// ClassCastException
		...
		if (parent instanceof Child) {	
			// parent 변수가 참조하는 객체가 Child 타입의 인스터스인지 검사
			// (Child 타입으로부터 만들어졌는지)
	...
	private static void methode1(Parent parent) {
		// Quiz: instanceof를 사용하여 안전하게 Child 타입으로 변환하기
		// 변환 성공시 "Child로 변환 성공" 출력
		// 변환 실패시 "Child로 변환되지 않음" 출력
		
		// 자바 12부터 추가된 문법
		// instanceof 경과가 ture일 경우 자동 타입 변환되어 변수에 대입
