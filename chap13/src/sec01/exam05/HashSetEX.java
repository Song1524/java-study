...
public class HashSetEX {
	...
	public static void main(String[] args) {
		...
		// 인스턴스는 다르지만 내부 테이터가 동일한 경우
		// 갹체의 hashCode()와 equals()를 기준으로 판단
		
		set.add(new Member("송진영", 24));
		set.add(new Member("송진영", 24));	// 동등객체
		
		set.remove(new Member("송진영", 24));
		...
		members.add(new Member("송진영", 24));
		members.add(new Member("홍길동", 20));
		members.add(new Member("김첨지", 28));
		...
		// 다시 Set으로 변환(순서 유지하려면 LinkedHashSet)
		...
		// Quiz
		Set<Member> memberSet = new HashSet<Member>(memList);
		
		List<Member> memLists = new ArrayList<Member>(memberSet);
