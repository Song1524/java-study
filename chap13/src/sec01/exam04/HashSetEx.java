...
// HashSet은 특정 상황에서만 사용되며, List 계열보다는 사용빈도가 낮은편 
...
public class HashSetEx {
	...
	public static void main(String[] args) {
		...
		// 만약 저장한 순서대로 꺼내왔다면 우연히 해시 출동이 없었거나,
		// JVM의 내부 해시 구현에 의해 우연히 정렬된 것처럼 츨력된 것일 뿐 순서를 보장하지 않음
		// 동일한 요소를 중복해서 저장할 수 없음(저장 자체를 안함)
		// 하나의 null만 저장 가능
		// 중복 판단 기준: hashCode()로 위치를 찾고, equals()로 동등성 확인
		...
		// 컬렉션의 요소를 하나씩 순회할 수 있도롣 도와주는 인터페이스
		...
		while (iterator.hasNext()) {	// 가져올 객체가 있으면 true, 없으면 false
			String element = iterator.next();	// 컬랙션에서 1개의 객체를 가져옴
			System.out.println("\t" + element);
			
			// iterator.remove(): next()로 반환한 요소를 제거, 보통 조건문과 같이 쓰임
		...
		// 객체의 hashCode()와 equals()를 기쥰으로 판단
		set.remove("JDBC");
		set.remove("iBatis");
...
//		System.out.println("교집합: " + class1);
		...
		// 요소 정렬
		// HashSet 자제는 정렬 기능이 없는 컬랙션이기 떄문에,
		// 정렬을 하려면 다른 컬랙션으로 변환한 뒤 정렬
...
//		Collections.sort(fruits); // 불가(List 계열만 가능)
		...
		// HashSet을 이용해 중복제거
		Set<String> javaClass = new HashSet<String>(list);	//중복제거 완료
