...
public class FileEx {
	...
	public static void main(String[] args) {
		...
		// 파일 객체를 생성했다고 해서 파일이나 폴더가 생성 되는 것은 아님
		// 문자열 경로에 실제 파일이나 폴더가 없더라도 예외가 발생하지 않음
		File dir = new File("C:/Temp/images");
		File file1 = new File("C:/Temp/file1.txt");
		File file2 = new File("C:/Temp/file2.txt");
		File file3 = new File("C:\\Temp\\file3.txt");	// \를 쓰려면 \\로 이스케이프
...
//		if (!dirTest.exists()) dirTest.mkdir();	// 경로상 맨뒤 폴더만 생성(앞에 상위 폴더가 있어야함)
		if (!dirTest.exists()) dirTest.mkdirs();// 경로상에 없는 모든 폴더 생생			
		...
		if (temp.exists() && temp.isDirectory()) {
			// Temp 폴더에 있는 파일 및 파일 폴더를 File 배열로 리턴
			File[] cotents = temp.listFiles();
			System.out.println("시간\t\t\t형태\t\t크기\t");
			System.out.println("---------------------------------------------------------------");
			...
			for (File file : cotents) {
				// 파일의 마지막 수정 날짜 확인
				// 마지막 수정 시간을 밀리초 단위로 반환
				// 날짜 형시긍로 바꾸기 위해 Date 클래스와 SimpleDatrFormat 활용
