...
public class WriteEx {
	...
	public static void main(String[] args) throws Exception {
		// 데이터 도작지를 test1.db 파일로 하는 바이트 기반 파일 출력 스트림을 생성
//		OutputStream os = new FileOutputStream("C:/Temp/test1.db");	// 바이트를 파일에 출력해서 저장하는 기능
...
//		byte c = 30;
		...
		os.write(b);
		os.write(c);
		// write()한다고 해서 바로 출력 스트림을 통해 데이터가 흘러가는 것이 아님
		// 버퍼(임시 저장 공산)에 쌓임
		// 버퍼: 물건을 잠시 담아두는 창고
		...
		// 더 이상 출력 스트림을 쓰지 않을 때는 출력 스트림 닫기
		os.close();	// 출력 스트림이 사용했던 자원들(메모리)을 풀어죽 위해
