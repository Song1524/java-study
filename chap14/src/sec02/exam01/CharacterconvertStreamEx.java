...
public class CharacterconvertStreamEx {
	// 파일도 문자를 저장하고, 저장된 문자를 다시 읽는 예제
	// 사용하는 소스 스트림(기본 스트림)은 바이트 기반 입출력 스트림이지만,
	// 이들 스트림을 직접 사용하지 않고 각각 Reader와 Wirter로 변환해서 사용
	...
	private static void write(String str) throws Exception {
		...
		writer.write(str);	// 문자열을 바로 쓸수 있음
		writer.flush();
		writer.close();
	...
	private static void read() throws Exception {
		...
		// 편의를 위해 보조 스트림을 연결새거 바이트 -> 문자로 자동 변환
		Reader reader = new InputStreamReader(is);	// 문자 기반 입력 보조 스트림 연결
		
		
		char[] buffer = new char[100];
		int readCharNum = reader.read(buffer);	// 보조 스트림연결로 char[]이용 가능
